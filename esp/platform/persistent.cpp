...
public:
    ...
    CPersistentInfo* findAndUse(SocketEndpoint* ep, PersistentProtocol proto)
    {
...
public:
    ...
    {
        CriticalBlock block(m_critsect);
        CPersistentInfo* info = m_availkeeper->findAndUse(ep, proto);
        if (info)
        {
            ...
            m_selectHandler->remove(info->sock);
